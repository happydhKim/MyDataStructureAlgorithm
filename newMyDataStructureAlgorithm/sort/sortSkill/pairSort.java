...
public class pairSort {
	...
	static void st_push(int v){st[sti++] = v;}
	static int st_pop(){ return st[--sti]; }
	static void qsrt(){
	    int sti = 0;
	    int t, p;
	    int i, j;
	    int l, r;
	    st_push(0); st_push(N - 1);
	    while (sti){
	        r = st_pop(), l = st_pop();
	        p = r, i = l - 1, j = r;
	        if (i < j){
	            ...
	            int xt = xy[i][0], yt = xy[i][1];
	            xy[i][0] = xy[p][0], xy[i][1] = xy[p][1];
	            xy[p][0] = xt, xy[p][1] = yt;
	            st_push(l), st_push(i - 1), st_push(i + 1), st_push(r);
	        }
